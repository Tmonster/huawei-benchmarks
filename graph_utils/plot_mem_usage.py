...
def plot_memory_data(benchmark_name):
    ...
    for i in range(22):
        ...
        hyper_cold_file = f"benchmarks/{benchmark_name}/{zfilled}_hyper_cold_mem.csv"
        duckdb_cold_file = f"benchmarks/{benchmark_name}/{zfilled}_duckdb_cold_mem.csv"
        hyper_hot_file = f"benchmarks/{benchmark_name}/{zfilled}_hyper_hot_mem.csv"
        duckdb_hot_file = f"benchmarks/{benchmark_name}/{zfilled}_duckdb_hot_mem.csv"
        
        if (not (os.path.exists(hyper_cold_file) and  os.path.exists(duckdb_cold_file) and os.path.exists(hyper_hot_file) and os.path.exists(duckdb_hot_file))):
            print(f"query {zfilled} did not complete for hyper or duckdb. Skipping plot")
            continue

        hyper_cold_df = pd.read_csv(hyper_cold_file, index_col=False)
        duckdb_cold_df = pd.read_csv(duckdb_cold_file, index_col=False)
        hyper_hot_df = pd.read_csv(hyper_hot_file, index_col=False)
        duckdb_hot_df = pd.read_csv(duckdb_hot_file, index_col=False)
        ...
        print(f"Saving cold plot for query {query_num}.")
        # Save the plot
        plt.savefig(f"benchmarks/{benchmark_name}/{zfilled}_cold_plot")
        plt.clf()
        ...
        print(f"Saving hot plot for query {query_num}.")
        # Save the plot
        plt.savefig(f"benchmarks/{benchmark_name}/{zfilled}_hot_plot")
        plt.clf()
