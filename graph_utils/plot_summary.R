...
for (benchmark_type in c('tpch', 'operators', 'tmm', 'tpcds')) {
  ...
  for (run_type in c('hot', 'cold')) {  
    ...
    dbExecute(con, sprintf("qcreate or replace temporary table hyper_start_times as select min(Time) as start_time, system, run_type, benchmark, benchmark_name, query_name as query from hyper_results where run_type = '%s' and benchmark = '%s' group by all", run_type, benchmark_type));
    ...
    results <- dbGetQuery(con, "select * from (select * FROM duckdb_results_x_y union all (select * from hyper_results_x_y)) where query not in (select query from single_point_queries)")
    if (run_type == 'hot') {
